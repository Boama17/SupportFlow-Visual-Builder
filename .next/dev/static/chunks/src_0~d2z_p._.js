...
function FlowInsightsPanel({ data }) {
    _s();
    const analytics = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "FlowInsightsPanel.useMemo[analytics]": ()=>{
            const nodeMap = new Map(data.nodes.map({
                "FlowInsightsPanel.useMemo[analytics]": (node)=>[
                        node.id,
                        node
                    ]
            }["FlowInsightsPanel.useMemo[analytics]"]));
            const startNode = data.nodes.find({
                "FlowInsightsPanel.useMemo[analytics].startNode": (node)=>node.type === 'start'
            }["FlowInsightsPanel.useMemo[analytics].startNode"]);
            const paths = [];
            const walk = {
                "FlowInsightsPanel.useMemo[analytics].walk": (nodeId, currentPath)=>{
                    const node = nodeMap.get(nodeId);
                    if (!node) return;
                    const nextPath = [
                        ...currentPath,
                    ...
                    node.children.forEach({
                        "FlowInsightsPanel.useMemo[analytics].walk": (childId)=>walk(childId, nextPath)
                    }["FlowInsightsPanel.useMemo[analytics].walk"]);
                }
            }["FlowInsightsPanel.useMemo[analytics].walk"];
            if (startNode) {
                walk(startNode.id, []);
            ...
            const totalPaths = paths.length || 1;
            const averagePathLength = paths.length ? Math.round(paths.reduce({
                "FlowInsightsPanel.useMemo[analytics]": (sum, path)=>sum + path.length - 1
            }["FlowInsightsPanel.useMemo[analytics]"], 0) / paths.length * 10) / 10 : 0;
            const reachCounts = Object.fromEntries(data.nodes.map({
                "FlowInsightsPanel.useMemo[analytics].reachCounts": (node)=>[
                        node.id,
                        0
                    ]
            }["FlowInsightsPanel.useMemo[analytics].reachCounts"]));
            paths.forEach({
                "FlowInsightsPanel.useMemo[analytics]": (path)=>{
                    path.forEach({
                        "FlowInsightsPanel.useMemo[analytics]": (nodeId)=>{
                            if (reachCounts[nodeId] !== undefined) {
                                reachCounts[nodeId] += 1;
                            }
                        }
            ...
            const nodeReach = Object.fromEntries(Object.entries(reachCounts).map({
                "FlowInsightsPanel.useMemo[analytics].nodeReach": ([nodeId, count])=>[
                        nodeId,
                        Math.round(count / totalPaths * 100)
                    ]
            }["FlowInsightsPanel.useMemo[analytics].nodeReach"]));
            const deadEnds = data.nodes.filter({
                "FlowInsightsPanel.useMemo[analytics].deadEnds": (node)=>node.children.length === 0 && node.type !== 'end'
            }["FlowInsightsPanel.useMemo[analytics].deadEnds"]).map({
                "FlowInsightsPanel.useMemo[analytics].deadEnds": (node)=>node.id
            }["FlowInsightsPanel.useMemo[analytics].deadEnds"]);
            const bottlenecks = data.nodes.filter({
                "FlowInsightsPanel.useMemo[analytics].bottlenecks": (node)=>{
                    const reach = nodeReach[node.id] ?? 0;
                    return reach < 35 && node.children.length > 0;
                }
            }["FlowInsightsPanel.useMemo[analytics].bottlenecks"]).map({
                "FlowInsightsPanel.useMemo[analytics].bottlenecks": (node)=>node.id
            }["FlowInsightsPanel.useMemo[analytics].bottlenecks"]);
            const maxDepth = paths.reduce({
                "FlowInsightsPanel.useMemo[analytics].maxDepth": (max, path)=>Math.max(max, path.length - 1)
            }["FlowInsightsPanel.useMemo[analytics].maxDepth"], 0);
            let healthScore = 100;
            healthScore -= deadEnds.length * 12;
            healthScore -= data.nodes.filter({
                "FlowInsightsPanel.useMemo[analytics]": (node)=>(nodeReach[node.id] ?? 0) < 10
            }["FlowInsightsPanel.useMemo[analytics]"]).length * 5;
            healthScore -= Math.max(0, maxDepth - 4) * 6;
            healthScore = Math.max(0, Math.min(100, healthScore));
            const suggestions = [];
            if (deadEnds.length > 0) {
            ...
            if (data.nodes.some({
                "FlowInsightsPanel.useMemo[analytics]": (node)=>node.children.length > 3
            }["FlowInsightsPanel.useMemo[analytics]"])) {
                suggestions.push('Split overcomplicated decision points that branch into too many options.');
            }
            if ((Object.values(nodeReach).filter({
                "FlowInsightsPanel.useMemo[analytics]": (value)=>value < 20
            }["FlowInsightsPanel.useMemo[analytics]"]).length || 0) > 0) {
                suggestions.push('Merge rarely reached branches to simplify the overall experience.');
            }
            const complexityWarning = maxDepth > 5 ? 'This flow is getting deep. Consider shortening the path to keep the conversation clear.' : maxDepth > 4 ? 'Decision depth is approaching the recommended limit; simplify if possible.' : null;
            const rankedNodes = data.nodes.map({
                "FlowInsightsPanel.useMemo[analytics].rankedNodes": (node)=>({
                        id: node.id,
                        label: getNodeLabel(node),
                        reach: nodeReach[node.id] ?? 0,
                        type: node.type
                    })
            }["FlowInsightsPanel.useMemo[analytics].rankedNodes"]).sort({
                "FlowInsightsPanel.useMemo[analytics].rankedNodes": (a, b)=>b.reach - a.reach
            }["FlowInsightsPanel.useMemo[analytics].rankedNodes"]);
            const heatmap = rankedNodes.map({
                "FlowInsightsPanel.useMemo[analytics].heatmap": (node)=>({
                        id: node.id,
                        label: node.label,
                        reach: node.reach,
                        tone: getTone(node.reach)
                    })
            }["FlowInsightsPanel.useMemo[analytics].heatmap"]);
            return {
                totalPaths,
                averagePathLength,
                healthScore,
...
_s(FlowInsightsPanel, "Q7AhsL3j2gcrWRU5TlKNAs9L+YU=");
...
//# sourceMappingURL=src_0~d2z_p._.js.map